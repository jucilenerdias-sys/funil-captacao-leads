...
export default function App() {
  ...
  // Digite aqui o número do novo chip (sem espaços, com DDD)
  ...
  // --- CAPTURA A FILIAL PELA URL (Ex: meusuite.com/?unidade=2) ---
  ...
  const perguntas = {
    ...
    5: {
      titulo: "Quantas pessoas moram debaixo do mesmo teto que você e fazem parte da sua família?",
      campo: "pessoas_casa",
      opcoes: [
...
${respostas.exclusao_renda ? `🚫 Desconto Legal: ${respostas.exclusao_renda}` : ''}
...
*Aviso ao Atendente:* Cliente triado automaticamente pela página. Entre em contato rápido para fechamento!`;
    ...
    const { error } = await supabase.from('tarefas').insert([{
      ...
      tipo: 'Prospecção Leads',       
      origem_prospeccao: 'leads',     
      status: 'Pendente',
    ...
    return (
      <div style={styles.container}>
        <div style={styles.card}>
          ...
          <p style={styles.texto}>Avaliamos as suas respostas e verificamos que você ainda não preenche os requisitos mínimos no momento.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={styles.container}>
        <div style={styles.card}>
          ...
          <p style={styles.texto}>Você <strong>tem grandes chances</strong> de conseguir o seu benefício ({tipoDireito})!</p>
          <a href={linkWhats} target="_blank" rel="noreferrer" style={styles.btnWhatsapp}>💬 Falar no WhatsApp</a>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={styles.container}>
        <div style={styles.card}>
          ...
          <p style={styles.texto}>O sistema encontrou um caminho para o seu caso. Precisamos apenas do seu nome e WhatsApp.</p>
          
          <div style={{ marginBottom: '15px', textAlign: 'left' }}>
            <label style={styles.label}>Qual o seu nome ou apelido?</label>
            <input style={styles.input} placeholder="Digite seu nome..." value={dadosLead.nome} onChange={e => setDadosLead({...dadosLead, nome: e.target.value})} />
          </div>
          <div style={{ marginBottom: '25px', textAlign: 'left' }}>
            <label style={styles.label}>Qual o seu WhatsApp com DDD?</label>
            <input style={styles.input} placeholder="(00) 00000-0000" value={dadosLead.telefone} onChange={handleTelefone} maxLength="15" />
          </div>
      ...
      <div style={styles.card}>
        ...
        <div style={styles.areaBotoes}>
          {perguntaObj.opcoes.map((op, idx) => (
            <button key={idx} onClick={() => processarResposta(perguntaObj.campo, op.valor)} style={styles.btnOpcao}>
              {op.label}
            </button>
          ))}
        </div>
...
const styles = {
  container: { minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#f3f4f6', fontFamily: 'system-ui, -apple-system, sans-serif', padding: '20px' },
  card: { backgroundColor: '#ffffff', padding: '40px 30px', borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)', maxWidth: '500px', width: '100%', textAlign: 'center' },
  barraFundo: { width: '100%', height: '8px', backgroundColor: '#e5e7eb', borderRadius: '4px', marginBottom: '5px', overflow: 'hidden' },
  barraProgresso: { height: '100%', backgroundColor: '#1e3a8a', transition: 'width 0.4s ease' },
  textoProgresso: { fontSize: '12px', color: '#6b7280', fontWeight: 'bold', textTransform: 'uppercase', display: 'block', marginBottom: '20px', textAlign: 'right' },
  tituloPergunta: { fontSize: '20px', color: '#111827', marginBottom: '30px', lineHeight: '1.4' },
  areaBotoes: { display: 'flex', flexDirection: 'column', gap: '12px' },
  btnOpcao: { padding: '16px 20px', fontSize: '16px', color: '#374151', backgroundColor: '#f8fafc', border: '2px solid #e2e8f0', borderRadius: '10px', cursor: 'pointer', textAlign: 'left', fontWeight: '600', transition: 'all 0.2s ease' },
  tituloPrincipal: { fontSize: '24px', color: '#1e3a8a', marginBottom: '15px' },
  tituloSecundario: { fontSize: '22px', color: '#111827', marginBottom: '15px', marginTop: '15px' },
  texto: { fontSize: '15px', color: '#4b5563', lineHeight: '1.5', marginBottom: '20px' },
  label: { fontSize: '14px', fontWeight: 'bold', color: '#374151', marginBottom: '8px', display: 'block', textAlign: 'left' },
  input: { width: '100%', padding: '15px', fontSize: '16px', borderRadius: '8px', border: '1px solid #d1d5db', backgroundColor: '#f9fafb', boxSizing: 'border-box', outline: 'none' },
  btnAcao: { width: '100%', padding: '16px', fontSize: '18px', backgroundColor: '#1e3a8a', color: '#ffffff', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' },
  btnWhatsapp: { display: 'block', width: '100%', padding: '16px', fontSize: '16px', backgroundColor: '#25D366', color: '#ffffff', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', textDecoration: 'none', boxSizing: 'border-box' },
  iconCircleRed: { width: '80px', height: '80px', borderRadius: '50%', backgroundColor: '#fee2e2', color: '#ef4444', fontSize: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto' },
  iconCircleGreen: { width: '80px', height: '80px', borderRadius: '50%', backgroundColor: '#dcfce7', color: '#10b981', fontSize: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto' }
};